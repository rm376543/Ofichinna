...
public sealed class DeleteServicoPecaCommandHandler : ICommandHandler<DeleteServicoPecaCommand, Result>
{
    ...
    public async Task<Result> HandleAsync(DeleteServicoPecaCommand command, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Iniciando desativação de peça de serviço. ServicoId: {ServicoId}, PecaId: {PecaId}.", command.ServicoId, command.PecaId);
            ...
            var peca = servico.ObterPeca(command.PecaId);
            if (peca is null || peca.EstaExcluida())
                return Result.Failure("Peça não encontrada.");

            servico.RemoverPeca(command.PecaId);

            await _unitOfWork.SaveChangesAsync();

            _logger.LogInformation("Peça de serviço desativada com sucesso. ServicoId: {ServicoId}, PecaId: {PecaId}.", command.ServicoId, command.PecaId);
            return Result.Success();
        }
        catch (DomainException ex)
        {
            _logger.LogWarning(ex, "Erro de domínio ao desativar peça de serviço. ServicoId: {ServicoId}, PecaId: {PecaId}.", command.ServicoId, command.PecaId);
            return Result.Failure(ex.Message);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Erro inesperado ao desativar peça de serviço. ServicoId: {ServicoId}, PecaId: {PecaId}.", command.ServicoId, command.PecaId);
