...
public sealed class ServicoTests
{
    ...
    {
        var servico = new Servico("Troca de óleo", "Substituição completa do óleo", 149.90m, true);
    ...
    {
        var servico = new Servico("Balanceamento", null, 89.90m, true);
    ...
    {
        Assert.Throws<DomainException>(() => new Servico(string.Empty, null, 10m, true));
        Assert.Throws<DomainException>(() => new Servico("Teste", null, 0m, true));
    }
}